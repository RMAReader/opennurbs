...
#pragma comment(lib, "C:\\Users\\Richard\\Documents\\Development\\opennurbs\\zlib\\Debug\\zlibx86.lib")
        ...
        )
{
  ...
  */
  ...
  while( rc && counter > 0 ) 
  {
	...
	zrc = 0;//deflate( &m_zlib.strm, flush ); 
    if ( zrc < 0 ) 
    {
      // Something went haywire - bail out.
        ...
        ) const
{
  ...
  while ( rc && counter > 0 )
  {
	...
	zrc = 0; // inflate(&m_zlib.strm, flush);
    if ( zrc < 0 ) 
    {
      // Something went haywire - bail out.
...
bool ON_CompressedBuffer::CompressionInit( struct ON_CompressedBufferHelper* helper ) const
{
  ...
  if ( helper )
  {
    ...
    if ( 1 == helper->action ) 
    {
      ...
      if ( Z_OK == 0)//deflateInit( &helper->strm, Z_BEST_COMPRESSION ) ) 
      {
        rc = true;
      }
      else 
    ...
    else if ( 2 == helper->action ) 
    {
      ...
      if ( Z_OK == 0)//inflateInit( &helper->strm ) ) 
      {
        rc = true;
      }
      else 
